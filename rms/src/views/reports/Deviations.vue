<template>
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-2">
            <div>
                <ol class="breadcrumb fs-sm mb-1">
                    ...
                    <li class="breadcrumb-item" aria-current="page">
                        <a href="javascript:void(0)">Inspection Deviations</a>
                    </li>
                    <!-- <li class="breadcrumb-item active" aria-current="page">Deviations</li> -->
                </ol>
                <h4 class="main-title mb-0">Inspection Deviations</h4>
            </div>
            <router-link to="/completed_deviations" class="btn btn-primary"> Completed Inspection Deviations</router-link>
            ...
            <div class="col-12">
                <div class="card card-one">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h6 class="card-title">Inspection Deviations</h6>
                        <!-- Moved Bulk Update Button to Right Side -->
                                ...
                                <tbody>
                                    ...
                                    <tr v-for="deviation, key in user_assets" :key="key">
                                        <td class="text-center">
                                            <input type="checkbox" :value="deviation.user_asset_check_id" v-model="selectedDeviations" @change="updateSelectAll" />
                                        </td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Update Inspection Deviation</h5>
                    <button type="button" id="close" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div>
                        <label class="form-label">Remarks</label><span class="text-danger"> *</span>
                        <textarea rows="2" class="form-control" type="text" placeholder="Remarks for Deviation" :class="{'is-invalid':errors.remarks}" v-model="user_check.remarks"></textarea>
    ...
    <div class="modal fade" id="bulkUpdateModal" tabindex="-1" aria-labelledby="bulkUpdateModalLabel" aria-hidden="true" ref="bulkDeviationModal">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="bulkUpdateModalLabel">Update Inspection Deviations</h5>
    ...
    export default {
        ...
        methods: {
            index() {
                ...
                vm.$store
                    .dispatch("post", { uri: "deviationAssetChecks", data: vm.meta })
                    .then((response) => {
                        loader.hide();
            ...
            bulkUpdateDeviation() {
                let vm = this;
                let loader = vm.$loading.show();
                const deviations = vm.user_assets
                    .filter((deviation) => vm.selectedDeviations.includes(deviation.user_asset_check_id))
                    .map((deviation) => ({
                    ...deviation,
                ...
                vm.$store
                    .dispatch("post", { uri: "bulkUpdateDeviation", data: data })
                    .then((response) => {
                        loader.hide();
                        vm.$store.dispatch("success", response.data.message);
            ...
            toggleSelectAll() {
            if (this.selectAll) {
                this.user_assets.forEach((deviation) => {
                if (!this.selectedDeviations.includes(deviation.user_asset_check_id)) {
                    this.selectedDeviations.push(deviation.user_asset_check_id);
                }
                });
            } else {
                this.user_assets.forEach((deviation) => {
                const index = this.selectedDeviations.indexOf(deviation.user_asset_check_id);
                if (index !== -1) {
            ...
            updateSelectAll() {
                this.selectAll =
                    this.user_assets.length > 0 &&
                    this.user_assets.every((deviation) =>
                    this.selectedDeviations.includes(deviation.user_asset_check_id)
                    );
            },
        },
    };
