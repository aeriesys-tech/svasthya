	...
	<div>
		...
		<div class="d-sm-flex align-items-center justify-content-between mb-2">
			<div>
				...
				<h4 class="main-title mb-0">TLC Ladle Daily Report</h4>
			</div>
			<div class="d-flex">
		...
		<div v-if="loaded">
			...
			<div class="card card-one mb-3">
				<div class="card-header py-2 d-flex align-items-center justify-content-between">
					<strong>Down TLC Details</strong>
					<small class="text-muted">
						({{ downColumns.length }} TLC{{ downColumns.length === 1 ? '' : 's' }})
					</small>
				</div>
				<div class="card-body">
					<div class="table-responsive table-responsive-sm">
			...
			<div class="card card-one mb-3">
				<div class="card-header py-2 d-flex align-items-center justify-content-between">
					<strong>Circulation TLC Details</strong>
					<small class="text-muted">
						({{ liveColumns.length }} TLC{{ liveColumns.length === 1 ? '' : 's' }})
					</small>
				</div>
				<div class="card-body">
					<div class="table-responsive table-responsive-sm">
...
export default {
	...
	methods: {
		...
		submit() {
			...
			this.$store.dispatch('post', { uri: 'getTLCDailyReport', data: this.meta })
				.then(res => {
					loader.hide();
					const data = res.data || {};
					this.report = data;
					this.report_details = data?.tlc_details ? JSON.parse(data.tlc_details) : [];
					this.down_stl_details = Array.isArray(data?.down_tlc_daily_asset_reports) ? data.down_tlc_daily_asset_reports : [];
					this.live_stl_details = Array.isArray(data?.live_tlc_daily_asset_reports) ? data.live_tlc_daily_asset_reports : [];
					this.loaded = true;
				})
				.catch(err => {
					loader.hide();
		...
		generateExcelData() {
			const workbook = {
				SheetNames: ['TLC Daily Report'],
				Sheets: { 'TLC Daily Report': {} }
			};

			let rowIndex = 1;
			const sheet = workbook.Sheets['TLC Daily Report'];

			// Title (kept for structure; styling handled in HTML)
			sheet['A' + rowIndex] = { v: 'TLC Ladle Daily Report', t: 's', s: { font: { bold: true, sz: 16 } } };
			rowIndex += 2;
			...
			// 1. Main
			sheet['A' + rowIndex] = { v: 'TLC Details', t: 's', s: { font: { bold: true, sz: 14 } } };
			rowIndex += 1;
			...
			if (this.downParsedUnique.length > 0) {
				sheet['A' + rowIndex] = { v: `Down TLC Details (${this.downColumns.length} TLC${this.downColumns.length === 1 ? '' : 's'})`, t: 's', s: { font: { bold: true, sz: 14 } } };
				rowIndex += 1;
			...
			if (this.liveParsedUnique.length > 0) {
				sheet['A' + rowIndex] = { v: `Circulation TLC Details (${this.liveColumns.length} TLC${this.liveColumns.length === 1 ? '' : 's'})`, t: 's', s: { font: { bold: true, sz: 14 } } };
				rowIndex += 1;
		...
		downloadExcelFile(workbook) {
			...
			link.download = `TLC_Daily_Report_${dateStr}.xls`;
  ...
  <body>`;
	...
	<div class="date-range">
	  ...
	  Generated on ${new Date().toLocaleString()} | RMS - Refractory Management System | JSW Steel
		...
		/* ---------- parsing & helpers ---------- */
		parseItem(item) {
			const arr = item?.tlc_details ? JSON.parse(item.tlc_details) : [];
			const byLabel = {};
	...
	computed: {
		...
		downParsedUnique() {
			...
			this.downParsed.forEach((p, i) => {
				const no = (p.byLabel['TLC No']?.value || p.byLabel['TLC  No']?.value || `TLC ${i + 1}`).toString();
		...
		downColumns() {
			return this.downParsedUnique.map((p, i) => {
				const v = p.byLabel['TLC No']?.value || p.byLabel['TLC  No']?.value;
				return v ? String(v) : `TLC ${i + 1}`;
			});
		},
		downRowLabels() {
