	...
	<div class="container px-3">
		...
		<div class="row gx-3 gx-lg-4">
			...
			<div class="col-12 mt-3">
				<button class="btn btn-lg btn-theme w-100 mb-3" @click="submitProfile" v-if="!loading">
					Update Profile
				</button>
	...
	authStore.get({
		...
		.then((response) => {
			...
			profile.value = {
				...
				currentWorking: userData.current_working || '',
				physicalHealth: '',
				qualification: userData.qualification || ''
			}
			// Update places based on division (without resetting currentWorking)
			if (profile.value.division) {
				currentPlaces.value = divisionPlaces[profile.value.division] || []
			}
		})
		.catch((error) => {
			if (error.response?.data?.message) {
				...
				toastRef.value?.show('Failed to load profile. Please try again.', TOAST_ERROR)
			}
		})
		.finally(() => {
			loading.value = false
...
function submitProfile() {
	if (!profile.value.name) {
		toastRef.value?.show('Name is required.', TOAST_ERROR)
		return
	}
	...
	authStore.post({
		...
		data: {
			...
			dob: profile.value.dob || null,
			phone: profile.value.phone || null,
			gender: profile.value.gender || null,
			designation: profile.value.designation || null,
			division: profile.value.division || null,
			current_working: profile.value.currentWorking || null,
			qualification: profile.value.qualification || null,
		},
		meta: { loadingRef: loading }
	})
		.then((response) => {
			toastRef.value?.show(response.data.message || 'Profile updated successfully!', TOAST_SUCCESS)
		...
		.catch((error) => {
			if (error.response?.data?.message) {
				...
				toastRef.value?.show('Failed to update profile. Please try again.', TOAST_ERROR)
			}
		})
		.finally(() => {
			loading.value = false
